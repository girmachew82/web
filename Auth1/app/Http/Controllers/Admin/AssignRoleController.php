...
class AssignRoleController extends Controller
{
    ...
    public function index()
    {
       ...
       $assignedroles = DB::table('role_user')
                        ...
                        ->where('role_user.user_id',2)
                        ->get();
                $no =1;
                $roles  = Role::all();

        return view('admin.pages.assignrole.index',['users'=>$users,'roles'=>$roles,'assignedroles'=>$assignedroles,'no'=>$no]);
    }
    ...
    public function assignRole (Request $request)
    {
        ...
        $assign->save();
        return redirect('/admin/pages/assignrole/index')->with('message', 'User has been assigned successfully');
